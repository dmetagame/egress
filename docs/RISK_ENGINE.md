...
The fetcher also accepts the exact OKX-owned `en-us` and `en-eu` canonical
paths for each source when OKX returns a geo-dependent redirect. Other locale
or path variants remain outside the allowlist.
